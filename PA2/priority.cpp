...
PixelPoint PriorityNeighbours::Remove() {
  PixelPoint priority = PixelPoint();
  int pri_index = -1;
  double temp_dist = 1000000000000000; // large to ensure first comparison works
 
  for (unsigned int i = 0; i < points.size(); i++) {
    double curr_dist = refcolor.dist(points[i].color);
    if (curr_dist < temp_dist) { 
      // dist always returns a positive value
      temp_dist = curr_dist;
      priority = points[i];
      pri_index = i;
    } else if (curr_dist == temp_dist) {
      // same priority value, choose minimal y-coordinate pixel
      PixelPoint curr_point = points[i];
      if (curr_point.y < priority.y) {
        priority = curr_point;
        pri_index = i;
      } else if (curr_point.y == priority.y) {
        // same priority value + same y-coordinate, choose minimal x-coordinate pixel
        if (curr_point.x < priority.x) {
          priority = curr_point;
          pri_index = i;
        }
      }
  ...
  // removes final priority pixel
  points.erase(points.begin() + pri_index);
